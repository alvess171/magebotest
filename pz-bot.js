(function () {
  ...
  const bot = {
    ...
    // Gera um WAV de bipe em memória. Elemento <audio> é bem mais
    // compatível que Web Audio: funciona em setups onde a saída do
    // AudioContext não chega no alto-falante.
    makeBeepWav(freq = 880, ms = 180, bips = 3, volume = 0.5) {
      const sampleRate = 8000;
      const totalMs = bips * (ms + 120);
      const n = Math.floor((sampleRate * totalMs) / 1000);
      ...
      const amostrasPorBip = Math.floor((sampleRate * ms) / 1000);
      const amostrasPorCiclo = Math.floor((sampleRate * (ms + 120)) / 1000);
      for (let i = 0; i < n; i++) {
        const pos = i % amostrasPorCiclo;
        let v = 128;
        if (pos < amostrasPorBip) {
          const onda = Math.sin((2 * Math.PI * freq * i) / sampleRate) >= 0 ? 1 : -1;
          v = 128 + Math.round(onda * 127 * Math.min(1, Math.max(0, volume)));
        }
        dv.setUint8(44 + i, v);
      }
      let bin = "";
    ...
    playAlarm(opcoes = {}) {
      const freq = Number(opcoes.freq) || 880;
      const bips = Number(opcoes.bips) || 3;
      const volume = opcoes.volume != null ? Number(opcoes.volume) : 0.5;

      // ── Caminho 1: elemento <audio> (mais confiável) ──
      try {
        // Se o elemento saiu do DOM (reconexão do jogo redesenha coisas)
        // ou entrou em erro, recria. Era isso que fazia o som "parar
        // de funcionar depois de um tempo".
        if (bot.__alarmEl && (!bot.__alarmEl.isConnected || bot.__alarmEl.error)) {
          try { bot.__alarmEl.remove(); } catch {}
          bot.__alarmEl = null;
        }
        if (!bot.__alarmEl) {
          const el = document.createElement("audio");
          el.id = "allInOne-alarm-audio";
          el.style.display = "none";
          document.body.appendChild(el);
          bot.__alarmEl = el;
        }
        const el = bot.__alarmEl;
        const chave = freq + "|" + bips + "|" + volume;
        if (el.__chave !== chave) {
          el.src = bot.makeBeepWav(freq, 180, bips, volume);
          el.__chave = chave;
        }
        el.volume = 1;
        el.currentTime = 0;
        const p = el.play();
        if (p?.catch) p.catch(() => bot.armAlarmUnlock());
      } catch (e) { log("playAlarm (audio el) falhou", e?.message || e); }

      // ── Caminho 2: Web Audio, como reforço ──
      try {
        const AC = window.AudioContext || window.webkitAudioContext;
        if (!AC) return;
        // Contexto fechado pelo navegador: recria em vez de falhar calado
        if (bot.__alarmCtx && bot.__alarmCtx.state === "closed") bot.__alarmCtx = null;
        ...
        if (ctx.state === "suspended") ctx.resume().then(tocar).catch(() => bot.armAlarmUnlock());
        else tocar();
      } catch (e) { log("playAlarm (web audio) falhou", e?.message || e); }
    },
    ...
    armAlarmUnlock() {
      ...
      const desbloquear = () => {
        try {
          ...
          // destrava também o elemento <audio> (toca mudo uma vez)
          if (bot.__alarmEl) {
            const v = bot.__alarmEl.volume;
            bot.__alarmEl.volume = 0;
            bot.__alarmEl.play?.().then(() => {
              bot.__alarmEl.pause();
              bot.__alarmEl.currentTime = 0;
              bot.__alarmEl.volume = v;
            }).catch(() => { bot.__alarmEl.volume = v; });
          }
    ...
    // Aviso dentro do painel. NÃO usa alert()/confirm(): esses congelam
    // o JavaScript da página inteira, o heartbeat do jogo não roda e a
    // conexão cai ("no server packets for 20s, forcing reconnect").
    toast(texto, cor) {
      try {
        let el = document.getElementById("allInOne-toast");
        if (!el) {
          el = document.createElement("div");
          el.id = "allInOne-toast";
          Object.assign(el.style, {
            position: "fixed", bottom: "16px", left: "50%",
            transform: "translateX(-50%)", background: "#1e1e1e",
            border: "1px solid #444", color: "#eee", padding: "8px 14px",
            borderRadius: "6px", fontSize: "12px", fontFamily: "sans-serif",
            zIndex: "9999999", pointerEvents: "none", maxWidth: "88vw",
            whiteSpace: "pre-line", textAlign: "center",
            boxShadow: "0 2px 10px rgba(0,0,0,0.5)", transition: "opacity .3s",
          });
          document.body.appendChild(el);
        }
        el.style.borderColor = cor || "#444";
        el.style.opacity = "1";
        el.textContent = texto;
        clearTimeout(el.__t);
        el.__t = setTimeout(() => { el.style.opacity = "0"; }, 4000);
      } catch {}
    },
    ...
    testAlarm() {
      bot.playAlarm();
      const r = {
        webAudio: bot.__alarmCtx?.state || "sem contexto",
        elementoAudio: bot.__alarmEl ? (bot.__alarmEl.paused ? "pausado" : "tocando") : "não criado",
        abaMuda: document.querySelector("audio")?.muted ? "sim" : "não",
      };
      console.table([r]);
      return r;
    },
...
window.__minibiaBotBundle.installAutoAttackModule = function installAutoAttackModule(bot) {
  ...
  const config = Object.assign(
    {
      ...
      chaseWoundedMobs: false, // perseguir mobs feridos que fogem
      woundedHealthPercent: 30, // considerar ferido abaixo deste %
      enabled: false,
    },
    storedConfig
  );
  ...
  function getEngagedTarget() {
    ...
    if (currentTarget) {
      if (!isAllowedTarget(currentTarget)) {
        // skipTarget só marca internamente — é preciso CANCELAR o alvo no
        // jogo, senão o personagem continua batendo no bicho ignorado.
        skipTarget(currentTarget, "not in target name list", Date.now(), 60000);
        clearCurrentTarget();
        clearCurrentFollowTarget();
        state.engagedTargetId = null;
        bot.log("auto attack: alvo ignorado — cancelando", { nome: currentTarget.name || "?" });
        return null;
      }
    ...
    // Se chaseWoundedMobs está ativado, procura por mobs feridos mesmo fora do raio
    if (config.chaseWoundedMobs) {
      const woundedTarget = findWoundedEngagedTarget();
      if (woundedTarget) {
        bot.log("auto attack: perseguindo mob ferido", { 
          id: woundedTarget.id, 
          nome: woundedTarget.name || "Mob"
        });
  ...
  // Detecta mobs feridos (com pouca vida) que fogem
  function isWoundedMob(creature) {
    if (!config.chaseWoundedMobs) return false;
    if (!creature) return false;
    
    const maxHealth = creature?.state?.maxHealth || creature?.maxHealth || creature?.maxHp || 100;
    const currentHealth = readCreatureHealth(creature);
    
    if (currentHealth <= 0 || maxHealth <= 0) return false;
    
    const healthPercent = (currentHealth / maxHealth) * 100;
    const woundedThreshold = Math.max(1, Math.min(99, Number(config.woundedHealthPercent) || 30));
    
    return healthPercent <= woundedThreshold;
  }

  // Procura por mobs feridos que já foram alvo para continuar perseguindo mesmo quando saem do raio
  function findWoundedEngagedTarget() {
    if (!config.chaseWoundedMobs || !state.engagedTargetId) return null;
    
    const allMonsters = (bot.xray?.getVisibleMonsters?.({ sameFloorOnly: false }) || [])
      .filter((m) => m && !config.skillTrainOnMonster || isReachableSkillTrainTarget(m, normalizePosition(bot.getPlayerPosition())));
    
    const woundedTarget = allMonsters.find((m) => m.id === state.engagedTargetId && isWoundedMob(m));
  ...
  function syncMeleeChase(now = Date.now()) {
    ...
    if (isAdjacentTile(playerPosition, targetPosition)) {
      ...
      const TENTATIVA_MS = 1000;  // insiste por 1s
      const DESCANSO_MS = 4000;   // descansa 4s atacando da diagonal

      if (!state.reposicionandoDesde) state.reposicionandoDesde = now;
      const decorrido = now - state.reposicionandoDesde;

      if (decorrido > TENTATIVA_MS + DESCANSO_MS) {
        // Recomeça o ciclo: a situação pode ter mudado (o player que
        // bloqueava saiu, o mob andou). Nunca desiste em definitivo.
        state.reposicionandoDesde = now;
        state.avisouReposicao = false;
      } else if (decorrido > TENTATIVA_MS) {
        // Em descanso: NÃO troca de alvo, só aceita a diagonal e continua
        // batendo. Volta a tentar o ortogonal depois.
        if (!state.avisouReposicao) {
          state.avisouReposicao = true;
          bot.log("auto attack: sem tile ortogonal livre — batendo da diagonal, tento de novo em " +
                  (DESCANSO_MS / 1000) + "s");
        }
        return false;
      }
...
window.__minibiaBotBundle.installCaveModule = function installCaveModule(bot) {
  ...
  const config = Object.assign(
    {
      ...
      waypointToleranceStrict: 3, // Tolerância aumentada quando strictOrder está ativado (evita "mancar")
  ...
  function contaParaPausa(criatura) {
    ...
    // SEMPRE verifica os ignorados do Attack, mesmo quando usarListaAttack está OFF
    // Isso evita contar mobs que estão na lista de ignorar do Attack
    const ignoradosAttack = normalizeNomes(bot.attack?.config?.ignoreNames);
    if (nome && ignoradosAttack.includes(nome)) return false;

    // Ignorados locais do Cave têm prioridade: quem está aqui nunca conta, mesmo
    // estando também na lista de permitidos.
    const ignoradosCave = normalizeNomes(config.pauseIgnoreNames);
    if (nome && ignoradosCave.includes(nome)) return false;
  ...
  function deveEsperarMobsAlcancarem() {
    ...
    // Respeita as listas: bicho ignorado não deve segurar a rota
    const monstros = (bot.xray?.getVisibleMonsters?.({ sameFloorOnly: true }) || []).filter(contaParaPausa);
  ...
  function getWaypointTolerance() {
    // Se strictOrder está ativado, usa uma tolerância maior pra evitar "mancar"
    if (config.strictOrder) {
      const value = Number(config.waypointToleranceStrict);
      if (!Number.isFinite(value) || value < 0) return 3;
      return Math.trunc(value);
    }
    
    // Modo normal (lookahead)
  ...
  function addWaypointCurrentSpot(options = {}) {
    ...
    // ── Proximity Skip ──
    // Só vale na GRAVAÇÃO AUTOMÁTICA (que dispara a cada 600ms e encheria
    // a rota de pontos repetidos). No botão manual não se aplica: se você
    // clicou, é porque quer aquele ponto ali mesmo, perto ou não.
    const aplicarProximidade = options.aplicarProximidade === true;

    if (config.proximitySkipEnabled && aplicarProximidade) {
  ...
  function startAutoRecord(intervalMs = 600) {
    ...
    autoRecordState.timerId = window.setInterval(() => {
      addWaypointCurrentSpot({ aplicarProximidade: true });
    }, intervalMs);
  ...
  bot.cave = {
    ...
    contarMobsPerto: () => getNearbyCreatures().length,
...
window.__minibiaBotBundle.installPanicModule = function installPanicModule(bot) {
  ...
  const config = Object.assign(
    {
      ...
      restoreDelaySec: 30,
      disableAttackOnPanic: true,  // desativar Attack quando panic dispara
      trustedNames: [],
      gameMasterNames: [],
    },
    bot.storage.get(configStorageKey, {})
  ...
  function restoreInterruptedModules() {
    ...
    // Reativa outros módulos que podem ter sido pausados
    if (state.pendingReturnModules?.attackRunning) {
      bot.attack?.start?.();
    }
  }

  // Verifica se pode restaurar módulos quando não há mais ameaça
  function checkAndRestoreIfSafe(now = Date.now()) {
    if (!state.pendingReturnModules) return false;
    
    const unknownPlayers = getUnknownVisiblePlayers();
    const visibleGameMasters = getVisibleGameMasters();
    
    // Se não há mais ameaça (players desconhecidos ou GMs), restaura
    if (!unknownPlayers.length && !visibleGameMasters.length) {
      bot.log("panic: ameaça passou, restaurando módulos", {
        pendingModules: state.pendingReturnModules
      });
  ...
  function triggerPanic(reason, details = {}) {
    ...
    // Desativa Attack se a opção estiver ativada
    if (config.disableAttackOnPanic && bot.attack?.stop) {
  ...
  function tick() {
    ...
    try {
      ...
      if (!triggered) {
        // Se nenhuma ameaça foi disparada, tenta restaurar módulos se a ameaça passou
        checkAndRestoreIfSafe();
        tryReturnToOrigin();
      }
    } finally {
      scheduleNextTick();
...
window.__minibiaBotBundle.installChatdetectorModule = function installChatdetectorModule(bot) {
  ...
  const defaultConfig = {
    ...
    tomHz: 880,
    qtdBips: 3,
    canaisPermitidos: ["Default", "Console"], // só estes dois são monitorados
  ...
  const config = Object.assign({}, defaultConfig, bot.storage.get(configStorageKey, {}), {
    // Canais são FIXOS no script: só Default e Console. Ignora qualquer
    // valor salvo por versões anteriores (inclusive a lista vazia que
    // significava "todos").
  ...
  function tocarAlarme() {
    // Delega pro alarme central do bot, que tenta <audio> E Web Audio.
    try {
      bot.playAlarm?.({
        freq: Number(config.tomHz) || 880,
        bips: Number(config.qtdBips) || 3,
  ...
  function processarMensagem(msgObj, nomeCanal, ehHistorico) {
    ...
    if (!bateuVigiadoPreCheck && deveIgnorar(mensagem, remetente)) {
      // A mensagem continua visível no chat do jogo — a lista de
      // ignorados serve só pra não disparar alarme.
      return;
    }
  ...
  // Ao reconectar, o cliente recria os canais e o nome do player pode
  // vir de novo. Sem re-sincronizar, o detector fica olhando pra um
  // estado velho e para de acusar mensagem.
  function resincronizarAposReconexao() {
    if (!config.enabled) return;
    window.setTimeout(() => {
      try {
        state.playerName = (window.gameClient?.player?.name || "").trim() || null;
        state.ultimaContagemPorCanal.clear();
        verificarCanais(true); // marca o que já existe como histórico

        // Se o loop tinha morrido, sobe de novo
        if (!state.running || state.timerId == null) {
          stop({ persistEnabled: false });
          start();
        }
        bot.log("chat detector: re-sincronizado após reconexão", {
          player: state.playerName,
          rodando: state.running,
        });
      } catch (erro) {
        bot.log("chat detector: falha ao re-sincronizar", erro?.message || erro);
      }
    }, 2000);
  }

  window.addEventListener("allInOne:reconnected", resincronizarAposReconexao);
  window.addEventListener("minibia:reconnected", resincronizarAposReconexao);

  // Vigia de segurança: se o módulo se diz ligado mas o loop sumiu,
  // religa sozinho. Cobre qualquer parada silenciosa.
  window.setInterval(() => {
    if (config.enabled && state.running && state.timerId == null) {
      bot.log("chat detector: loop tinha parado — religando");
      stop({ persistEnabled: false });
      start();
    }
  }, 10000);
...
window.__minibiaBotBundle.installTalkModule = function installTalkModule(bot) {
  ...
  const defaultConfig = {
    ...
    apiKeys: [],   // chaves extras pro rodízio quando a cota de uma acabar
    model: defaultModel,
    pollMs: minPollMs,
    replyCooldownMs: 1500,
    responderTodos: true,   // true = responde qualquer um, inclusive GM
    naoResponder: [],       // nomes que o Talk NUNCA responde
  ...
  const config = Object.assign(
    ...
    defaultConfig,
    storedConfig,
    // Talk sempre começa DESLIGADO e SEM chave nenhuma. As chaves valem
    // só pra sessão atual — recarregou, entra limpo.
    { enabled: false, apiKey: "", apiKeys: [] }
  );

  // ── RODÍZIO DE CHAVES ──────────────────────────────────────
  // A cota do Gemini é por PROJETO/dia. Com várias chaves de projetos
  // diferentes, dá pra continuar quando uma esgota.
  function getTodasChaves() {
    const lista = [];
    if (config.apiKey) lista.push(String(config.apiKey).trim());
  ...
  // Cota da Google zera à meia-noite do Pacífico (~08:00 UTC)
  function proximoResetDaCota() {
    const agora = new Date();
    const reset = new Date(Date.UTC(
      agora.getUTCFullYear(), agora.getUTCMonth(), agora.getUTCDate(), 8, 0, 0
    ));
    if (reset.getTime() <= agora.getTime()) reset.setUTCDate(reset.getUTCDate() + 1);
    return reset.getTime();
  }

  function chaveEstaEsgotada(chave) {
    const ate = state.chavesEsgotadas?.[chave];
    if (!ate) return false;
    if (Date.now() >= ate) { delete state.chavesEsgotadas[chave]; return false; }
    return true;
  }

  function marcarChaveEsgotada(chave, ate) {
    if (!chave) return;
    state.chavesEsgotadas = state.chavesEsgotadas || {};
    state.chavesEsgotadas[chave] = ate || proximoResetDaCota();
  }

  function getChaveAtiva() {
    const chaves = getTodasChaves();
    for (const c of chaves) if (!chaveEstaEsgotada(c)) return c;
    return null; // todas esgotadas
  }

  function statusDasChaves() {
    return getTodasChaves().map((c, i) => ({
      indice: i,
      final: "..." + c.slice(-6),
      esgotada: chaveEstaEsgotada(c),
      voltaEm: state.chavesEsgotadas?.[c] ? new Date(state.chavesEsgotadas[c]).toLocaleString() : null,
    }));
  }

  function addApiKey(chave) {
    const v = String(chave || "").trim();
    if (!v || getTodasChaves().includes(v)) return false;
    if (!config.apiKey) config.apiKey = v;
    else config.apiKeys = [...(config.apiKeys || []), v];
    persistConfig();
    return true;
  }

  function removeApiKey(chave) {
    if (config.apiKey === chave) {
      const resto = (config.apiKeys || []);
      config.apiKey = resto[0] || "";
      config.apiKeys = resto.slice(1);
    } else {
      config.apiKeys = (config.apiKeys || []).filter((k) => k !== chave);
    }
    delete state.chavesEsgotadas?.[chave];
    persistConfig();
    return true;
  }

  function persistConfig() {
    // As API keys NÃO vão pro localStorage de propósito: ficam só na
    // memória da sessão. Pra guardar, use "💾 Salvar em arquivo" na aba.
    const paraSalvar = { ...config };
    delete paraSalvar.apiKey;
    delete paraSalvar.apiKeys;
    bot.storage.set(configStorageKey, paraSalvar);
  }
  ...
  function sanitizeConfig() {
    ...
    config.naoResponder = Array.isArray(config.naoResponder)
      ? config.naoResponder.map((n) => String(n || "").trim()).filter(Boolean)
      : [];
    config.apiKeys = Array.isArray(config.apiKeys)
      ? config.apiKeys.map((k) => String(k || "").trim()).filter(Boolean)
      : [];
  ...
  // Lista pessoal do Talk: quem estiver aqui nunca recebe resposta.
  // Independente do "Responder a todos" — esta trava sempre vale.
  function estaNaListaDeIgnorados(message) {
    const nome = normalizeText(message?.sender);
    if (!nome) return false;
    return (config.naoResponder || []).some((n) => normalizeText(n) === nome);
  }

  function addNaoResponder(nome) {
    const v = String(nome || "").trim();
    if (!v) return false;
    if ((config.naoResponder || []).some((n) => normalizeText(n) === normalizeText(v))) return false;
    config.naoResponder = [...(config.naoResponder || []), v];
    persistConfig();
    return true;
  }

  function removeNaoResponder(nome) {
    config.naoResponder = (config.naoResponder || []).filter((n) => n !== nome);
    persistConfig();
    return true;
  }
  ...
  function getNewestPendingMessage() {
    const pendingMessages = getDefaultMessages().filter((message) => {
      ...
      if (!message.sender || isSelfMessage(message) || isNpcMessage(message) || isTrustedSender(message) || isGameMasterSender(message) || estaNaListaDeIgnorados(message)) {
  ...
  async function generateText(prompt, generationConfig = {}) {
    const chaveUsada = getChaveAtiva();
    if (!chaveUsada) {
      state.cotaDiariaEsgotada = true;
      throw new Error("Todas as API keys estão com a cota esgotada");
    }

    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(config.model)}:generateContent?key=${encodeURIComponent(chaveUsada)}`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      ...
      if (response.status === 429 || response.status === 503) {
        ...
        if (cotaDiaria) {
          // Marca ESTA chave como esgotada até o reset e tenta a próxima.
          marcarChaveEsgotada(chaveUsada);
          const proxima = getChaveAtiva();

          if (proxima) {
            espera = 0; // já tenta de novo com a chave nova
            motivo = "trocando de chave";
            state.backoffAte = 0;
            bot.log("talk: cota da chave ..." + chaveUsada.slice(-6) + " acabou — indo pra próxima", {
              restantes: statusDasChaves().filter((k) => !k.esgotada).length,
            });
          } else {
            espera = Math.max(60000, proximoResetDaCota() - Date.now());
            motivo = "todas as chaves esgotadas";
            state.cotaDiariaEsgotada = true;
            bot.log("talk: TODAS as chaves estão sem cota — desligando. " +
                    "A cota zera à meia-noite do Pacífico (~5h no Brasil). " +
                    "Pra ter mais: crie chaves em PROJETOS diferentes do Google Cloud.");
            bot.playAlarm?.();
  ...
  async function maybeRespond() {
    if (!state.running || state.pending || !config.enabled || !getChaveAtiva()) {
      return false;
    }
  ...
  function start(overrides = {}) {
    ...
    if (!getChaveAtiva()) {
      bot.log("Talk: sem API Key disponível — configure na aba Talk (ou todas estão sem cota)");
  ...
  // Limpa chaves que ficaram gravadas por versões anteriores do script
  try {
    const salvo = bot.storage.get(configStorageKey, {}) || {};
    if (salvo.apiKey || (salvo.apiKeys && salvo.apiKeys.length)) {
      delete salvo.apiKey; delete salvo.apiKeys;
      bot.storage.set(configStorageKey, salvo);
      bot.log("talk: chaves antigas removidas do armazenamento (agora ficam só na sessão)");
    }
  } catch {}

  console.log("[Talk Module] API Key: ⚠ nenhuma — carregue na aba Talk (arquivo ou colando)");
...
window.__minibiaBotBundle.installautostackModule = function installautostackModule(bot) {
  ...
  // Confirmação em 2 cliques, sem confirm(): o confirm() congela o
  // JavaScript e derruba a conexão do jogo.
  function pedirConfirmacao(botao, textoConfirmar, acao) {
    if (botao.__armado) {
      clearTimeout(botao.__t);
      botao.__armado = false;
      botao.textContent = botao.__textoOriginal;
      botao.style.background = botao.__bgOriginal;
      acao();
      return;
    }
    botao.__armado = true;
    botao.__textoOriginal = botao.__textoOriginal || botao.textContent;
    botao.__bgOriginal = botao.__bgOriginal || botao.style.background;
    botao.textContent = textoConfirmar;
    botao.style.background = "#a33";
    botao.__t = setTimeout(() => {
      botao.__armado = false;
      botao.textContent = botao.__textoOriginal;
      botao.style.background = botao.__bgOriginal;
    }, 4000);
  ...
  function buildProfilesTab() {
    ...
    updateBtn.onclick = () => {
      ...
      if (!activeName) { bot.toast("Nenhum perfil ativo. Carregue um, ou use 'Salvar como novo perfil'."); return; }
    ...
    saveBtn.onclick = () => {
      if (!nameInput.value.trim()) { bot.toast("Digite um nome pro perfil."); return; }
    ...
    loadBtn.onclick = () => {
      ...
      pedirConfirmacao(loadBtn, "Confirmar? substitui tudo", () => Profiles.load(listEl2.value));
    };
    ...
    deleteBtn.onclick = () => {
      if (!listEl2.value || listEl2.disabled) return;
      pedirConfirmacao(deleteBtn, "Confirmar exclusão?", () => {
    ...
    importBtn.onclick = () => {
      ...
      if (!nome) { bot.toast("Não consegui ler esse perfil."); return; }
      importArea.value = "";
      refreshList();
      bot.toast("Perfil \"" + nome + "\" importado. Selecione e clique em Carregar.");
    };
  ...
  function buildAttackTab() {
    ...
    // ── Perseguir mobs feridos que fogem ──
    const woundedRow = el("label", "display:flex; align-items:center; gap:6px; margin-bottom:4px; cursor:pointer; color:#ccc;");
    const woundedCheckbox = el("input");
    woundedCheckbox.type = "checkbox";
    woundedCheckbox.checked = !!bot.attack.config.chaseWoundedMobs;
    woundedCheckbox.onchange = () => bot.attack.updateConfig({ chaseWoundedMobs: woundedCheckbox.checked });
    woundedRow.appendChild(woundedCheckbox);
    woundedRow.appendChild(document.createTextNode("Perseguir mobs feridos que fogem"));
    wrap.appendChild(woundedRow);

    const woundedWrap = el("div", "margin:0 0 8px 20px;");
    woundedWrap.appendChild(makeField("↳ Considerar ferido abaixo de (%) vida", bot.attack.config.woundedHealthPercent ?? 30, (v) => {
      bot.attack.updateConfig({ woundedHealthPercent: Math.max(1, Math.min(99, Number(v) || 30)) });
    }, "number"));
    woundedWrap.appendChild(el("div", "color:#666; font-size:10px; font-style:italic; line-height:1.5;", "Quando um mob atingir este % de vida, o bot continuará perseguindo mesmo que saia do raio normal."));
    wrap.appendChild(woundedWrap);
  ...
  function buildCaveTab() {
    ...
    clearWpBtn.onclick = () => pedirConfirmacao(clearWpBtn, "Confirmar limpeza?", () => { bot.cave.clearWaypoints(); renderBody(); });
    ...
    const proxRow = el("label", "display:flex; align-items:center; gap:6px; margin-bottom:4px; cursor:pointer; color:#ccc; font-size:11px;");
    const proxCheckbox = el("input");
    proxCheckbox.type = "checkbox";
    proxCheckbox.checked = !!bot.cave.config.proximitySkipEnabled;
    proxCheckbox.onchange = () => bot.cave.updateConfig({ proximitySkipEnabled: proxCheckbox.checked });
    proxRow.appendChild(proxCheckbox);
    proxRow.appendChild(document.createTextNode("Na gravação auto, pular pontos muito próximos"));
    wrap.appendChild(proxRow);

    const proxWrap = el("div", "margin:0 0 8px 20px;");
    proxWrap.appendChild(makeField("↳ Distância mínima (sqm)", bot.cave.config.minProximitySkip ?? 3, (v) => {
      bot.cave.updateConfig({ minProximitySkip: Number(v) || 3 });
    }, "number"));
    proxWrap.appendChild(el("div", "color:#666; font-size:10px; font-style:italic; line-height:1.5;", "Vale SÓ pra gravação automática (que grava a cada 0,6s e encheria a rota de pontos repetidos). O botão \"+ Waypoint aqui\" nunca é bloqueado."));
    wrap.appendChild(proxWrap);
    ...
    // Campo de tolerância para Strict Order
    const strictToleranceWrap = el("div", "margin:0 0 8px 20px;");
    strictToleranceWrap.appendChild(makeField("↳ Tolerância chegada (sqm)", bot.cave.config.waypointToleranceStrict ?? 3, (v) => {
      bot.cave.updateConfig({ waypointToleranceStrict: Math.max(1, Math.min(15, Number(v) || 3)) });
    }, "number"));
    strictToleranceWrap.appendChild(el("div", "color:#666; font-size:10px; font-style:italic; line-height:1.5;", "Aumentar este valor evita \"mancar\" quando Strict Order está ligado. Padrão: 3 sqm"));
    wrap.appendChild(strictToleranceWrap);
  ...
  function buildGmPanicTab() {
    ...
    const disableAttackRow = el("label", "display:flex; align-items:center; gap:6px; margin-bottom:8px; cursor:pointer; color:#ccc;");
    const disableAttackCheckbox = el("input");
    disableAttackCheckbox.type = "checkbox";
    disableAttackCheckbox.checked = !!bot.panic.config.disableAttackOnPanic;
    disableAttackCheckbox.onchange = () => bot.panic.updateConfig({ disableAttackOnPanic: disableAttackCheckbox.checked });
    disableAttackRow.appendChild(disableAttackCheckbox);
    disableAttackRow.appendChild(document.createTextNode("Desativar Attack quando dispara"));
    wrap.appendChild(disableAttackRow);
  ...
  function buildTalkTab() {
    ...
    // ── Lista de chaves (rodízio) ──
    const chavesListEl = el("div", "max-height:76px; overflow-y:auto; margin-bottom:6px; background:#111; border-radius:4px; padding:4px;");
    wrap.appendChild(chavesListEl);

    function renderChaves() {
      chavesListEl.innerHTML = "";
      const lista = bot.talk.statusDasChaves();
      if (!lista.length) {
        chavesListEl.appendChild(el("div", "color:#e77; font-size:11px;", "(nenhuma chave configurada)"));
        return;
      }
      lista.forEach((k, i) => {
        const row = el("div", "display:flex; justify-content:space-between; align-items:center; padding:2px 0; font-size:11px;");
        const cor = k.esgotada ? "#e77" : (i === 0 ? "#5c5" : "#999");
        const marca = k.esgotada ? "🚫" : (i === 0 ? "▶" : "•");
        const txt = el("span", "color:" + cor + ";", marca + " chave " + k.final + (k.esgotada ? " (sem cota)" : ""));
        row.appendChild(txt);
        const rm = el("span", "color:#e77; cursor:pointer; padding:0 4px;", "✕");
        rm.onclick = () => {
          const todas = bot.talk.statusDasChaves();
          const alvo = (bot.talk.config.apiKey && i === 0)
            ? bot.talk.config.apiKey
            : (bot.talk.config.apiKeys || [])[i - (bot.talk.config.apiKey ? 1 : 0)];
          if (alvo) { bot.talk.removeApiKey(alvo); renderChaves(); bot.toast("Chave " + k.final + " removida"); }
        };
        row.appendChild(rm);
        chavesListEl.appendChild(row);
      });
    }
    renderChaves();

    const chaveRow = el("div", "display:flex; gap:4px; margin-bottom:4px;");
    const chaveInput = el("input", "flex:1; padding:4px; border-radius:4px; border:1px solid #444; background:#2a2a2a; color:#eee;");
    chaveInput.placeholder = "colar nova API key";
    const addChaveBtn = el("button", "padding:4px 10px; border:none; border-radius:4px; background:#2d7a2d; color:#fff; cursor:pointer;", "+");
    addChaveBtn.onclick = () => {
      if (bot.talk.addApiKey(chaveInput.value)) { chaveInput.value = ""; renderChaves(); }
      else bot.toast("Chave vazia ou já cadastrada.");
    };
    chaveInput.onkeydown = (e) => { if (e.key === "Enter") addChaveBtn.click(); };
    chaveRow.appendChild(chaveInput);
    chaveRow.appendChild(addChaveBtn);
    wrap.appendChild(chaveRow);

    wrap.appendChild(el("div", "color:#666; font-size:10px; font-style:italic; margin-bottom:6px; line-height:1.5;", "▶ = em uso agora. Quando a cota de uma acabar, ele passa pra próxima sozinho. Crie cada chave num PROJETO diferente do Google Cloud — chaves do mesmo projeto dividem a mesma cota."));

    // ── Importar várias chaves de um arquivo ──
    function importarChaves(texto, origem) {
      // Aceita uma por linha, separadas por vírgula, ponto-e-vírgula ou espaço.
      // Ignora linhas de comentário (# ou //) pra você poder anotar no arquivo.
      ...
      let novas = 0, repetidas = 0, invalidas = 0;
      brutos.forEach((k) => {
        // chave do Gemini é longa; descarta lixo tipo "key:" ou números soltos
        if (k.length < 20) { invalidas++; return; }
        if (bot.talk.addApiKey(k)) novas++; else repetidas++;
      });
      ...
      bot.toast(
        "Importado de " + origem + ":\n\n" +
        "✅ novas: " + novas + "\n" +
        "• já cadastradas: " + repetidas + "\n" +
        (invalidas ? "⚠ ignoradas (curtas demais): " + invalidas + "\n" : "") +
        "\nTotal na lista: " + bot.talk.statusDasChaves().length
      );
    }

    const fileInput = el("input", "display:none;");
    fileInput.type = "file";
    fileInput.accept = ".txt,.csv,.json,text/plain";
    fileInput.onchange = () => {
      const arquivo = fileInput.files?.[0];
      if (!arquivo) return;
      const leitor = new FileReader();
      leitor.onload = () => {
        let conteudo = String(leitor.result || "");
        // Aceita também JSON: ["chave1","chave2"] ou {"keys":[...]}
        try {
          const j = JSON.parse(conteudo);
          if (Array.isArray(j)) conteudo = j.join("\n");
          else if (Array.isArray(j?.keys)) conteudo = j.keys.join("\n");
        } catch {}
        importarChaves(conteudo, arquivo.name);
        fileInput.value = "";
      };
      leitor.onerror = () => bot.toast("Não consegui ler o arquivo.");
      leitor.readAsText(arquivo);
    };
    wrap.appendChild(fileInput);

    const arquivoBtnRow = el("div", "display:flex; gap:4px; margin-bottom:6px;");

    const abrirBtn = el("button", "flex:1; padding:5px; border:none; border-radius:4px; background:#2c4fc7; color:#fff; cursor:pointer; font-size:11px;", "📂 Carregar de arquivo");
    abrirBtn.onclick = () => fileInput.click();

    const salvarBtn = el("button", "flex:1; padding:5px; border:none; border-radius:4px; background:#333; color:#ccc; cursor:pointer; font-size:11px;", "💾 Salvar em arquivo");
    salvarBtn.onclick = () => {
      const cfg = bot.talk.config;
      const todas = [cfg.apiKey, ...(cfg.apiKeys || [])].filter(Boolean);
      if (!todas.length) { bot.toast("Nenhuma chave pra salvar."); return; }
      const conteudo = "# API keys do Talk — uma por linha\n" + todas.join("\n") + "\n";
    ...
    arquivoBtnRow.appendChild(abrirBtn);
    arquivoBtnRow.appendChild(salvarBtn);
    wrap.appendChild(arquivoBtnRow);

    // ── Colar várias de uma vez ──
    const colarArea = el("textarea", "width:100%; height:50px; margin-bottom:4px; padding:4px; border-radius:4px; border:1px solid #444; background:#2a2a2a; color:#eee; font-size:10px; box-sizing:border-box;");
    colarArea.placeholder = "ou cole várias chaves aqui (uma por linha)";
    wrap.appendChild(colarArea);

    const colarBtn = el("button", "width:100%; padding:5px; margin-bottom:6px; border:none; border-radius:4px; background:#2d7a2d; color:#fff; cursor:pointer; font-size:11px;", "📋 Importar as chaves coladas acima");
    colarBtn.onclick = () => {
      if (!colarArea.value.trim()) { bot.toast("Cole as chaves primeiro."); return; }
      importarChaves(colarArea.value, "texto colado");
      colarArea.value = "";
    };
    wrap.appendChild(colarBtn);

    wrap.appendChild(el("div", "color:#e9a; font-size:10px; line-height:1.5; background:#111; border-radius:4px; padding:6px; margin-bottom:8px;", "🔒 As chaves NÃO ficam salvas: valem só até você recarregar a página. Use \"💾 Salvar em arquivo\" pra guardar e \"📂 Carregar de arquivo\" pra repor rapidinho."));

    wrap.appendChild(el("div", "color:#666; font-size:10px; font-style:italic; margin-bottom:8px; line-height:1.5;", "Arquivo .txt com uma chave por linha (ou .json com uma lista). Linhas começando com # são ignoradas, então dá pra anotar de qual projeto é cada uma."));
    ...
    // ── Lista de quem NÃO responder ──
    wrap.appendChild(el("div", "color:#ccc; font-size:11px; margin:8px 0 3px; border-top:1px solid #333; padding-top:8px;", "Nunca responder estes players:"));

    const ignoradosEl = el("div", "max-height:70px; overflow-y:auto; margin-bottom:6px; background:#111; border-radius:4px; padding:4px;");
    wrap.appendChild(ignoradosEl);

    function renderIgnorados() {
      ignoradosEl.innerHTML = "";
      const lista = bot.talk.config.naoResponder || [];
      if (!lista.length) {
        ignoradosEl.appendChild(el("div", "color:#666; font-size:11px; font-style:italic;", "(nenhum)"));
        return;
      }
      lista.forEach((nome) => {
        const row = el("div", "display:flex; justify-content:space-between; align-items:center; padding:2px 0; font-size:11px;");
        row.appendChild(el("span", null, nome));
        const rm = el("span", "color:#e77; cursor:pointer; padding:0 4px;", "✕");
        rm.onclick = () => { bot.talk.removeNaoResponder(nome); renderIgnorados(); };
        row.appendChild(rm);
        ignoradosEl.appendChild(row);
      });
    }
    renderIgnorados();

    const ignRow = el("div", "display:flex; gap:4px; margin-bottom:4px;");
    const ignInput = el("input", "flex:1; padding:4px; border-radius:4px; border:1px solid #444; background:#2a2a2a; color:#eee;");
    ignInput.placeholder = "nome do player";
    const addIgnBtn = el("button", "padding:4px 10px; border:none; border-radius:4px; background:#2d7a2d; color:#fff; cursor:pointer;", "+");
    addIgnBtn.onclick = () => {
      if (bot.talk.addNaoResponder(ignInput.value)) { ignInput.value = ""; renderIgnorados(); }
    };
    ignInput.onkeydown = (e) => { if (e.key === "Enter") addIgnBtn.click(); };
    ignRow.appendChild(ignInput);
    ignRow.appendChild(addIgnBtn);
    wrap.appendChild(ignRow);

    wrap.appendChild(el("div", "color:#666; font-size:10px; font-style:italic; margin-bottom:8px; line-height:1.5;", "Vale mesmo com \"Responder a todos\" marcado — esta lista sempre tem prioridade. Não diferencia maiúscula de minúscula."));
    ...
    testModelBtn.onclick = async () => {
      const chave = bot.talk.getChaveAtiva();
      if (!chave) { bot.toast("Configure a API Key primeiro."); return; }
  ...
  function updatePanelInner() {
    ...
    if (lureStatusEl && bot.cave) {
      try {
        ...
        // Usa o contador do próprio módulo, que JÁ aplica as listas de
        // permitidos/ignorados. Contar aqui de novo dava número diferente.
        const qtd = bot.cave.contarMobsPerto ? bot.cave.contarMobsPerto() : 0;
    ...
    if (talkStatusEl && bot.talk) {
      ...
      if (s.cotaDiariaEsgotada) {
        talkStatusEl.textContent = "🚫 Todas as chaves sem cota (" + bot.talk.statusDasChaves().length + ")";
