...
window.__minibiaBotBundle.installautostackModule = function installautostackModule(bot) {
  ...
  function buildChatTab() {
    ...
    somBtn.onclick = () => {
      ...
      setTimeout(() => {
        ...
        alert(ok ? "🔊 Som liberado — o alarme já funciona." : "🔇 Ainda bloqueado. Verifique se a aba não está muda e o volume do sistema.");
        updatePanel();
      }, 400);
    };
  ...
  function buildTalkTab() {
    ...
    function renderChaves() {
      ...
      lista.forEach((k, i) => {
        ...
        rm.onclick = () => {
          ...
          if (alvo && confirm("Remover a chave " + k.final + "?")) { bot.talk.removeApiKey(alvo); renderChaves(); }
        };
        row.appendChild(rm);
    ...
    addChaveBtn.onclick = () => {
      ...
      else alert("Chave vazia ou já cadastrada.");
    };
    ...
    fileInput.onchange = () => {
      ...
      leitor.onerror = () => alert("Não consegui ler o arquivo.");
    ...
    salvarBtn.onclick = () => {
      ...
      if (!todas.length) { alert("Nenhuma chave pra salvar."); return; }
    ...
    colarBtn.onclick = () => {
      if (!colarArea.value.trim()) { alert("Cole as chaves primeiro."); return; }
