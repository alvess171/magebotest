(function () {
  ...
  // ===== MÓDULO: BACKGROUND (mantém a aba viva em segundo plano) =====
  const Background = (() => {
    const KEY = "background.config";
    const config = Object.assign({ audioEnabled: false, wakeLockEnabled: false }, bot.storage.get(KEY, {}));
    const state = {
      audioEl: null,
      audioOn: false,
      wakeLock: null,
    ...
    // Gera um WAV quase silencioso (amplitude de 1 LSB em 8 bits).
    // Silêncio digital puro às vezes é ignorado pelo navegador na hora
    // de decidir se "tem mídia tocando" — um sinal mínimo é mais confiável
    // e continua inaudível.
    function makeKeepAliveWav(seconds = 2) {
      ...
      for (let i = 0; i < numSamples; i++) {
        view.setUint8(44 + i, i % 2 === 0 ? 128 : 129); // ±1 LSB, inaudível
      }
      let binary = "";
    ...
    function startAudio() {
      ...
      try {
        if (!state.audioEl) {
          ...
          audio.id = "allInOne-keepalive-audio";
          audio.src = makeKeepAliveWav(2);
          audio.loop = true;
          audio.volume = 0.001;
        ...
        if (p?.then) {
          p.then(() => {
            ...
            log("background: áudio keep-alive tocando");
            updatePanel();
          }).catch((e) => {
            // Autoplay bloqueado — arma pro próximo toque do usuário
            state.audioOn = false;
            armGesture();
            log("background: áudio bloqueado, aguardando um toque", e?.message || e);
            updatePanel();
          });
        } else {
          state.audioOn = true;
        ...
        log("background: falha ao iniciar áudio", e?.message || e);
        return false;
      }
    }
    ...
    function onGesture() {
      if (config.audioEnabled && !state.audioOn) startAudio();
      if (config.wakeLockEnabled && !state.wakeOn) requestWake();
      if (state.audioOn || !config.audioEnabled) disarmGesture();
    }
    ...
    function disarmGesture() {
      state.pendingGesture = false;
      document.removeEventListener("touchend", onGesture, { passive: true });
      document.removeEventListener("mousedown", onGesture, { passive: true });
    }

    // ── Wake Lock: impede a tela de apagar ──────────────────────
    function isWakeSupported() { return "wakeLock" in navigator; }

    async function requestWake() {
      if (!isWakeSupported() || state.wakeLock) return false;
      try {
        ...
        log("background: wake lock ativo (tela não apaga)");
        updatePanel();
        return true;
      } catch (e) {
        state.wakeOn = false;
        log("background: wake lock recusado", e?.message || e);
        return false;
      }
    }
    ...
    // O navegador solta o wake lock sozinho quando a aba sai de foco —
    // aqui a gente pede de volta assim que ela volta.
    function onVisibility() {
      if (document.visibilityState === "visible") {
        if (config.wakeLockEnabled && !state.wakeLock) requestWake();
        if (config.audioEnabled && !state.audioOn) startAudio();
      }
    }
    document.addEventListener("visibilitychange", onVisibility);

    // Batimento: se os timers forem estrangulados em segundo plano,
    // o intervalo real fica muito maior que o pedido — isso aparece no status.
    ...
    function stopHeartbeat() {
      if (state.heartbeatId != null) { window.clearInterval(state.heartbeatId); state.heartbeatId = null; }
    }

    function setAudio(value) {
      config.audioEnabled = !!value;
      persist();
      if (config.audioEnabled) { startAudio(); startHeartbeat(); }
      else { stopAudio(); if (!config.wakeLockEnabled) stopHeartbeat(); }
      return config.audioEnabled;
    }

    function setWakeLock(value) {
      config.wakeLockEnabled = !!value;
      persist();
      if (config.wakeLockEnabled) { requestWake(); startHeartbeat(); }
      else { releaseWake(); if (!config.audioEnabled) stopHeartbeat(); }
      return config.wakeLockEnabled;
    }

    if (config.audioEnabled || config.wakeLockEnabled) {
      startHeartbeat();
      if (config.audioEnabled) startAudio();
      if (config.wakeLockEnabled) requestWake();
    }
    ...
    return {
      config, setAudio, setWakeLock, isWakeSupported,
      get audioOn() { return state.audioOn; },
      get wakeOn() { return state.wakeOn; },
      get pendingGesture() { return state.pendingGesture; },
      get lastHeartbeatAt() { return state.lastHeartbeatAt; },
      get running() { return state.audioOn || state.wakeOn; },
    };
  })();

  // ===== MÓDULO: GAME AUDIO (silencia o som SEM parar de tocar) =====
  // O jogo usa o próprio áudio pra segurar a página viva em segundo plano.
  // Pausar mataria esse keep-alive, e no iOS `volume` é ignorado por JS.
  // A saída é rotear tudo por um GainNode em 0: pro sistema continua
  // "tocando mídia", mas o que sai é silêncio.
  const GameAudio = (() => {
    const KEY = "gameAudio.config";
    const config = Object.assign({ muted: false }, bot.storage.get(KEY, {}));
    const state = { gains: [], elementSources: new WeakMap(), observer: null, patched: false, ctx: null };

    function persist() { bot.storage.set(KEY, { ...config }); }

    const origConnect = AudioNode.prototype.connect;

    function targetGainValue() { return config.muted ? 0 : 1; }

    function makeGainFor(ctx) {
      const gain = ctx.createGain();
      gain.gain.value = targetGainValue();
      gain.__allInOneBypass = true; // não reroteia o próprio gain
    ...
    function getGainFor(ctx) {
      let gain = ctxGains.get(ctx);
      if (!gain) { gain = makeGainFor(ctx); ctxGains.set(ctx, gain); }
      return gain;
    }

    // Intercepta qualquer nó que se conecte na saída de áudio
    function patchConnect() {
      if (state.patched) return;
      state.patched = true;
      AudioNode.prototype.connect = function (dest, ...rest) {
        try {
          const isDestination = typeof AudioDestinationNode !== "undefined" && dest instanceof AudioDestinationNode;
          // __allInOneBypass marca o alarme do próprio bot — esse NÃO é silenciado
          if (isDestination && !this.__allInOneBypass && dest.context) {
            const gain = getGainFor(dest.context);
    ...
    function getOwnCtx() {
      ...
      if (!AC) return null;
      try { state.ctx = new AC(); } catch { return null; }
      return state.ctx;
    }

    // Elementos <audio>/<video> só dá pra controlar no iOS passando
    // por um MediaElementSource — volume/muted diretos não obedecem.
    function routeMediaElement(elm) {
      if (!elm || state.elementSources.has(elm)) return;
      const ctx = getOwnCtx();
      if (!ctx) return;
      try {
        const source = ctx.createMediaElementSource(elm);
        const gain = getGainFor(ctx);
        origConnect.call(source, gain);
        state.elementSources.set(elm, source);
      } catch (e) {
        // já roteado por outro código, ou cross-origin — ignora
      }
    }

    function routeAllMedia() {
      document.querySelectorAll("audio, video").forEach(routeMediaElement);
    }
      ...
      state.observer = new MutationObserver((mutations) => {
        ...
        mutations.forEach((m) => {
          m.addedNodes?.forEach((node) => {
            ...
            if (node.tagName === "AUDIO" || node.tagName === "VIDEO") routeMediaElement(node);
            node.querySelectorAll?.("audio, video").forEach(routeMediaElement);
          });
        });
      });
    ...
    function applyGain() {
      const value = targetGainValue();
      state.gains.forEach((gain) => {
        try { gain.gain.value = value; } catch {}
      });
    }
    ...
    function setMuted(value) {
      ...
      applyGain();
      log(config.muted ? "game audio: silenciado (continua tocando pro sistema)" : "game audio: som de volta");
      updatePanel();
      return config.muted;
    }
    ...
    return {
      config, setMuted, routeAllMedia,
      get muted() { return config.muted; },
      get routedCount() { return state.gains.length; },
      get running() { return config.muted; },
    };
  })();
...
window.__minibiaBotBundle.installTalkModule = function installTalkModule(bot) {
  ...
  // ===== ABA: FUNDO (mantém rodando em segundo plano, evita desconectar) =====
  function buildFundoTab() {
    const wrap = el("div");

    wrap.appendChild(el("div", "color:#999; font-size:11px; margin-bottom:8px;", "O navegador estrangula timers de abas em segundo plano — é o que costuma derrubar a conexão. Estas opções seguram a aba viva."));

    const statusEl = el("div", "margin-bottom:8px; font-size:11px;");
    statusEl.dataset.fundoStatus = "1";
    wrap.appendChild(statusEl);

    // ── Áudio keep-alive ──
    wrap.appendChild(el("div", "color:#ccc; font-size:12px; font-weight:bold; margin-bottom:4px;", "🔊 Áudio silencioso (keep-alive)"));
    wrap.appendChild(el("div", "color:#999; font-size:11px; margin-bottom:6px;", "Toca um som inaudível em loop. Enquanto há mídia tocando, o navegador não estrangula os timers da aba."));

    const audioRow = el("label", "display:flex; align-items:center; gap:6px; margin-bottom:8px; cursor:pointer; color:#ccc; font-size:11px;");
    const audioCheckbox = el("input");
    audioCheckbox.type = "checkbox";
    audioCheckbox.checked = !!Background.config.audioEnabled;
    audioCheckbox.onchange = () => Background.setAudio(audioCheckbox.checked);
    audioRow.appendChild(audioCheckbox);
    audioRow.appendChild(document.createTextNode("Ativar áudio keep-alive"));
    wrap.appendChild(audioRow);

    wrap.appendChild(el("div", "border-top:1px solid #333; margin:10px 0 6px;"));

    // ── Wake Lock ──
    wrap.appendChild(el("div", "color:#ccc; font-size:12px; font-weight:bold; margin-bottom:4px;", "💡 Manter a tela ligada"));

    if (Background.isWakeSupported()) {
      wrap.appendChild(el("div", "color:#999; font-size:11px; margin-bottom:6px;", "Impede a tela de apagar sozinha. Com a tela ligada o jogo nunca é suspenso."));

      const wakeRow = el("label", "display:flex; align-items:center; gap:6px; margin-bottom:8px; cursor:pointer; color:#ccc; font-size:11px;");
      const wakeCheckbox = el("input");
      wakeCheckbox.type = "checkbox";
      wakeCheckbox.checked = !!Background.config.wakeLockEnabled;
      wakeCheckbox.onchange = () => Background.setWakeLock(wakeCheckbox.checked);
      wakeRow.appendChild(wakeCheckbox);
      wakeRow.appendChild(document.createTextNode("Não deixar a tela apagar"));
      wrap.appendChild(wakeRow);
    } else {
      wrap.appendChild(el("div", "color:#e9a; font-size:11px; margin-bottom:8px;", "Wake Lock não suportado neste navegador — use o áudio acima e/ou aumente o tempo de bloqueio de tela nos ajustes do aparelho."));
    }

    wrap.appendChild(el("div", "border-top:1px solid #333; margin:10px 0 6px;"));

    // ── Silenciar o som do jogo (sem quebrar o keep-alive) ──
    wrap.appendChild(el("div", "color:#ccc; font-size:12px; font-weight:bold; margin-bottom:4px;", "🔇 Silenciar o som do jogo"));
    wrap.appendChild(el("div", "color:#999; font-size:11px; margin-bottom:6px;", "Deixa o áudio do jogo mudo SEM pausar — pro sistema ele continua tocando, então o keep-alive do próprio cliente segue funcionando."));

    const muteRow = el("label", "display:flex; align-items:center; gap:6px; margin-bottom:6px; cursor:pointer; color:#ccc; font-size:11px;");
    const muteCheckbox = el("input");
    muteCheckbox.type = "checkbox";
    muteCheckbox.checked = !!GameAudio.muted;
    muteCheckbox.onchange = () => GameAudio.setMuted(muteCheckbox.checked);
    muteRow.appendChild(muteCheckbox);
    muteRow.appendChild(document.createTextNode("Silenciar o jogo (mantendo tocando)"));
    wrap.appendChild(muteRow);

    wrap.appendChild(el("div", "color:#666; font-size:10px; font-style:italic; margin-bottom:8px;", "Os alarmes do bot (GM Panic e Chat) continuam audíveis — só o som do jogo é cortado."));

    wrap.appendChild(el("div", "border-top:1px solid #333; margin:10px 0 6px;"));
    wrap.appendChild(el("div", "color:#666; font-size:10px; font-style:italic; line-height:1.5;",
      "Limite real: se você trocar de app ou bloquear a tela, o iOS suspende a página de qualquer jeito — o áudio ajuda, mas não é garantia. O caminho mais confiável é manter a tela ligada e o jogo em primeiro plano."));
  ...
  const tabBuilders = {
    ...
    tela: buildTelaTab, fundo: buildFundoTab, misc: buildMiscTab,
  };
  ...
  function updatePanelInner() {
    ...
    // ── Fundo (keep-alive) ──
    const fundoStatusEl = bodyEl.querySelector("[data-fundo-status]");
    if (fundoStatusEl) {
      const partes = [];
      if (Background.audioOn) partes.push("áudio");
      if (Background.wakeOn) partes.push("tela ligada");
      if (GameAudio.muted) partes.push("jogo mudo");
      if (partes.length) {
        fundoStatusEl.textContent = "● Ativo: " + partes.join(" + ");
        fundoStatusEl.style.color = "#5c5";
      } else if (Background.pendingGesture) {
        fundoStatusEl.textContent = "⏳ Toque na tela pra liberar o áudio";
        fundoStatusEl.style.color = "#fc5";
      } else {
        fundoStatusEl.textContent = "○ Parado";
        fundoStatusEl.style.color = "#999";
      }
    }
